...
This is separate from the agent's own `notifiers` (Discord, Slack, email, and the rest) that deliver [its threshold alerts](/docs/notifications) — those run on the server and can't fire once it goes offline, which is exactly why the heartbeat is delivered from Lookout instead.
